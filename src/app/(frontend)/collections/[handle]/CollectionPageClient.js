...
import { ChevronDown, ChevronRight, XIcon, ChevronsDown, Hammer, Filter as FilterIcon, LayoutDashboard, ShoppingBag, Loader2, ListFilter, ArrowUpDown, LayoutGrid, X, SlidersHorizontal, Check } from "lucide-react";
...
// Recently Viewed horizontal row with an always-visible 2px theme scrollbar.
// The native scrollbar is hidden and replaced by a custom indicator whose thumb
...
function RecentlyViewedRow({ products }) {
  ...
  useEffect(() => { updateThumb(); }, [updateThumb, products]);
  ...
  return (
    <div className="bg-[#FEF5F1]/60 rounded-[10px] px-4 py-4">
      ...
      >
        {products.map((item) => {
          const imgSrc = item.images?.[0]?.url || item.image?.url || item.image;
    ...
    Object.entries(filtersData || {}).forEach(([groupKey, options]) => {
      if (groupKey === "Price") {
        ...
        options.forEach((opt) => {
          ...
          if (groupKey === "Metal Color") {
            if (label === "R" || label.toLowerCase() === "rose") label = "Rose Gold";
            if (label === "Y" || label.toLowerCase() === "yellow") label = "Yellow Gold";
            if (label === "W" || label.toLowerCase() === "white") label = "White Gold";
          }
          if (mergedOptionsMap.has(label)) {
            const existing = mergedOptionsMap.get(label);
            existing.count += opt.count || 0;
          } else {
            mergedOptionsMap.set(label, { ...opt, label, value: opt.value !== undefined ? opt.value : label });
          }
        });
  ...
  useEffect(() => {
    ...
    async function fetchTrueBounds() {
      try {
        ...
        console.error("Failed to fetch true price bounds:", e);
      }
    }
    fetchTrueBounds();
  ...
  const gridItems = useMemo(() => {
    ...
    products.forEach((prod, idx) => {
      ...
      if (
        ...
        recentlyViewedAt = renderedCount;
        items.push(
    ...
    <div className="min-h-screen bg-white">
      ...
      <div ref={productsTopRef} className={`scroll-mt-20 ${isMobile ? "" : "flex xl:gap-12 lg:gap-6 py-6 container-main mx-auto"}`}>
        ...
        <div className="flex-1">
          ...
          <div className={`grid mt-4 transition-opacity duration-300 ${productsLoading ? "opacity-50 pointer-events-none" : ""} ${isMobile ? "grid-cols-2 gap-4 px-2" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 min-[1920px]:grid-cols-4 gap-6"}`}>
