...
function StoreListContent({ stores, selectedStoreId, onSelect, onUseMyLocation }) {
  return (
    <div className="p-6 space-y-6">
      <button onClick={onUseMyLocation} className="flex items-center gap-2 text-sm font-medium text-zinc-700 hover:underline">
        <Navigation size={16} />
        Use my location
      </button>

      <div className="space-y-3">
        <p className="text-sm text-zinc-500">There are {stores.length} locations with your item</p>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-lg border-none">
          ...
          <div className="p-4 bg-zinc-50 border-t border-zinc-100 flex items-center justify-end gap-3">
            <Button variant="outline" onClick={() => onOpenChange(false)} className="px-8 h-12 border-zinc-200 text-zinc-600 font-bold">
              Cancel
            </Button>
            <Button onClick={onSave} className="px-10 h-12 bg-primary hover:bg-primary/90 text-white font-bold">
              Save
            </Button>
          </div>
        </DialogContent>
