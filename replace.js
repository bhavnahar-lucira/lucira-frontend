const fs = require('fs');
const content = fs.readFileSync('src/app/(checkout-flow)/(protected)/checkout/payment/page.js', 'utf-8');

const newMobileBlock =             {!isDesktop && (
...
const startMarker = "{!isDesktop && (";
const endMarker = "{/* DESKTOP ONLY ORDER */}";

const startIdx = content.indexOf(startMarker);
const endIdx = content.indexOf(endMarker);

if (startIdx !== -1 && endIdx !== -1) {
    const newContent = content.substring(0, startIdx) + newMobileBlock + "\n\n            " + content.substring(endIdx);
    fs.writeFileSync('src/app/(checkout-flow)/(protected)/checkout/payment/page.js', newContent, 'utf-8');
